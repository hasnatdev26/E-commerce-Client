...
const EditProduct = () => {
    ...
    useEffect(() => {
        axiosPublic.get(`/products/${id}`).then((res) => {
            ...
            setKeyFeatures(data.keyFeatures?.length ? data.keyFeatures : [""]);
            setPrice(data.price);
    ...
    const addFeatureField = () => {
        if (keyFeatures.length >= 5) {
            toast.warning("Maximum 5 key features allowed");
            return;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (productImages.some((img) => img)) {
                if (productImages.some((img) => !img)) {
                    toast.error(
                        "Please upload all three product images at the same time"
                    );
                    setIsLoading(false);
                    return;
                }
            ...
            const updatedProduct = {
                ...
                keyFeatures: keyFeatures.filter(Boolean).slice(0, 5),
                images,
                categoryImage,
            };
            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <main className="w-full md:w-3/4 bg-white shadow-md rounded-lg p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                            ...
                            {sizes.map((s, i) => (
                                <input
                                    ...
                                    value={s}
                                    onChange={(e) =>
                                        handleSizeChange(i, e.target.value)
                                    }
                                    className={inputClass}
                        ...
                        <button
                            disabled={isLoading}
                            className="w-full bg-blue-600 text-white py-2 rounded-md hover:bg-blue-700 focus:ring-2 focus:ring-blue-400 focus:outline-none disabled:opacity-60"
                        >
